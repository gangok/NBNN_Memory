...
{
	dps.Initialize_From_Array(arr, nData, dim);
	//REAL_TYPE** q_arr;

	// Select first n data to query
	/*q_arr = new REAL_TYPE * [ NUM_QUERY ];
	for(int i = 0; i < NUM_QUERY; i++)
	{
		q_arr[i] = new REAL_TYPE [ dim ];
	}
	for(int i = 0; i < NUM_QUERY; i++)
	{
		for(int k = 0; k < dim; k++)
	...
	qps.Initialize_From_Array(q_arr, nQData, dim);
...
bitset<BCODE_LEN>* Generate_LSH_bCode()
{
	...
	printf("- Learning Spherical Hashing Finished\n");
	...
	printf("- LSH: Computing Binary Codes Finished\n");

	return bCodeData_LSH;
}
...
#pragma omp parallel for
	...
	printf("- Spherical Hashing: Computing Binary Codes Finished\n");
...
bitset<BCODE_LEN>* get_SH_code_test()
{
	...
	bitset<BCODE_LEN> *bCodeData_SH = new bitset<BCODE_LEN>[nQ];
...
#pragma omp parallel for
	...
	for (int i = 0; i<nQ; i++)
	{
		sh.Compute_BCode(qps.d[i], bCodeData_SH[i]);
	}
